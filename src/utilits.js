...
export const scroll_ = () => {
  const sections = document.querySelectorAll(".tr__section");
...
export const stickyNav = () => {
  ...
  const stickys = document.querySelectorAll(".tr__header");
...
export const portfolioHover = () => {
  const tr__portfolio_animation_wrap = document.querySelectorAll(
      ".tr__portfolio_animation_wrap"
    ),
    tr__portfolio_titles = document.querySelector(
      ".tr__portfolio_titles"
    );
  tr__portfolio_animation_wrap.forEach((element) => {
    element.addEventListener("mousemove", () => {
      let title = element.getAttribute("data-title"),
        category = element.getAttribute("data-category");
      if (title) {
        tr__portfolio_titles.classList.add("visible");
        tr__portfolio_titles.innerHTML =
          title + '<span class="work__cat">' + category + "</span>";
      }
      document.addEventListener("mousemove", (e) => {
        tr__portfolio_titles.style.left = `${e.clientX - 10}px`;
        tr__portfolio_titles.style.top = `${e.clientY + 25}px`;
      });
    });
    element.addEventListener("mouseleave", () => {
      tr__portfolio_titles.classList.remove("visible");
    });
  });
};
